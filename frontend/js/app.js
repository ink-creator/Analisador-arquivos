(() => {
  ...
  function renderCodeAnalysis(code) {
    const summary = code.summary || {};
    const files = code.files || [];
    const dependencies = code.dependencies || { edges: [], cycles: [] };

    byId("code-python-files").textContent = formatNumber(summary.files || 0);
    byId("code-functions").textContent = formatNumber(summary.functions || 0);
    byId("code-classes").textContent = formatNumber(summary.classes || 0);
    byId("code-average-complexity").textContent = new Intl.NumberFormat(locale(), {
      minimumFractionDigits: 1,
      maximumFractionDigits: 2,
    }).format(summary.average_complexity || 0);
    byId("code-parsed-summary").textContent = t("parsedFiles", {
      parsed: formatNumber(summary.parsed_files || 0),
      total: formatNumber(summary.files || 0),
    });

    renderCodeFiles(files);

    const hotspots = files
      .filter((file) => file.most_complex_function)
      .map((file) => ({
        ...file.most_complex_function,
        path: file.path,
      }))
      .sort((a, b) => b.complexity - a.complexity || a.path.localeCompare(b.path))
      .slice(0, 10)
      .map((item) => ({
        name: `${item.qualified_name} · ${item.path}:${item.line}`,
        value: formatNumber(item.complexity),
      }));
    renderRankList(byId("complex-functions-list"), hotspots);

    const edges = dependencies.edges || [];
    byId("dependency-edge-count").textContent = t("dependencyCount", {
      count: formatNumber(edges.length),
    });
    if (edges.length === 1) {
      byId("dependency-edge-count").textContent = t("dependencyCountOne", { count: "1" });
    }
    renderDependencies(edges);

    const cycles = dependencies.cycles || [];
    byId("dependency-cycle-count").textContent = t("cycleCount", {
      count: formatNumber(cycles.length),
    });
    if (cycles.length === 1) {
      byId("dependency-cycle-count").textContent = t("cycleCountOne", { count: "1" });
    }
    renderCycles(cycles);
  }

  function renderCodeFiles(files) {
    const container = byId("code-files-list");
    ...
    files.forEach((file) => {
      ...
      metrics.className = "code-file-metrics";
      if (file.parse_error) {
        metrics.appendChild(metricChip(t("parseErrorMetric", { error: file.parse_error }), "parse-error"));
      } else {
        metrics.append(
          metricChip(t(file.functions === 1 ? "functionMetricOne" : "functionMetric", { count: formatNumber(file.functions) })),
          metricChip(t(file.classes === 1 ? "classMetricOne" : "classMetric", { count: formatNumber(file.classes) })),
          metricChip(t("complexityMetric", { value: formatNumber(file.average_complexity) })),
        );
        if (file.most_complex_function) {
          metrics.appendChild(metricChip(t("mostComplexMetric", {
            name: file.most_complex_function.qualified_name,
            value: formatNumber(file.most_complex_function.complexity),
          })));
        }
      }
  ...
  function metricChip(label, extraClass = "") {
    const chip = document.createElement("span");
    chip.className = `metric-chip ${extraClass}`.trim();
  ...
  function renderDependencies(edges) {
    ...
    if (!edges.length) {
      const empty = document.createElement("div");
      empty.className = "empty-inline";
      empty.textContent = t("noLocalDependencies");
      container.appendChild(empty);
      return;
    }
    ...
    edges.forEach((edge) => {
      if (!targetsBySource.has(edge.source)) targetsBySource.set(edge.source, []);
    ...
    targetsBySource.forEach((targets, source) => {
      ...
      sourceLabel.className = "dependency-source";
      sourceLabel.textContent = source;
      item.appendChild(sourceLabel);
      targets.forEach((target) => {
        const targetLabel = document.createElement("span");
        targetLabel.className = "dependency-target";
  ...
  function renderCycles(cycles) {
    const container = byId("dependency-cycles-list");
    const panel = byId("dependency-cycles-panel");
    container.replaceChildren();
    panel.classList.toggle("no-cycles", !cycles.length);
    if (!cycles.length) {
      const empty = document.createElement("div");
      empty.className = "empty-inline";
      empty.textContent = t("noCircularDependencies");
      container.appendChild(empty);
      return;
    }
    ...
    cycles.forEach((cycle) => {
      ...
      item.textContent = cycle.cycle.join(" → ");
      container.appendChild(item);
    });
  }
